...
class PermisosActivity : AppCompatActivity() {
    ...
    private fun irAHome() {
        lifecycleScope.launch {
            try {
                CustomDialog.loadingDialog(this@PermisosActivity)
                CustomDialog.showDialog("Verificando", "Espere...", CustomDialog.DialogType.LOADING)
                ...
                if (!tieneCasas) {
                    startActivity(Intent(this@PermisosActivity, com.example.android.feature.auth.InitialSetupActivity::class.java))
                } else {

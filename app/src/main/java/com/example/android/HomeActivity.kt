...
class HomeActivity : AppCompatActivity() {
    ...
    private fun actualizarNombreUsuario() {
        findViewById<TextView>(R.id.tvUsuario)?.let {
            ...
            it.text = sharedPref.getString("userName", "Mayordomo")
        }
    }
    ...
    private fun sincronizarDatosServidor() {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                val dispositivosResponse = RetrofitClient.deviceService.getDispositivos(bearer)
                if (dispositivosResponse.isSuccessful) {
                    val dispositivos = dispositivosResponse.body()?.data ?: emptyList()
                    db.dispositivoDao().deleteAllDispositivos()
                    db.dispositivoDao().insertAll(dispositivos)
                }

                val gestosResponse = RetrofitClient.gestureService.getGestos(bearer)
                if (gestosResponse.isSuccessful) {
                    val gestos = gestosResponse.body()?.data ?: emptyList()
